...
namespace CollectionMathTests
{
    ...
    public class MathCollectionTests
    {
        ...
        public void FindRoots_TwoRoots_DiscriminantGreaterThanZero()
        {
            ...
            Assert.AreEqual(expected1, actual[0], delta, $"Ожидаемый корень 1 для a={a}, b={b}, c={c}: {expected1}");
            Assert.AreEqual(expected2, actual[1], delta, $"Ожидаемый корень 2 для a={a}, b={b}, c={c}: {expected2}");
        }
        ...
        public void FindRoots_OneRoot_DiscriminantEqualToZero()
        {
            ...
            Assert.AreEqual(expected1, actual[0], delta, $"Ожидаемый корень для a={a}, b={b}, c={c}: {expected1}");
        }
